...
function Step2Interview({interviewData , onFinish}){
  ...
  useEffect(() => {
    const loadVoices = () => {
        ...
        const maleVoice = 
          ...
          //Fallback: Fisrt voice (assume female)
  ...
  useEffect(() => {
    ...
    const runIntro = async () => {
        if(isIntroPhase){
            await speakText(
                `Hi ${userName} , it's a great to meet you today. I hope you're feeling confident and ready.`
            );
    ...
    return(
        <div className="min-h-screen bg-linear-to-br from-emerald-50 via-white to-teal-100 flex items-center justify-center p-4 sm:p-6">
            ...
            <textarea placeholder="Type your answer here..."
               onChange={(e) => setAnswer(e.target.value)} value={answer} className="flex-1 bg-gray-100 p-4 sm:p-6 rounded-2xl resize-none outline-none border border-gray-200 focus:ring-2 focus:ring-emerald-500 transition text-gray-800"/>
            ...
            {!feedback ? (<div className="flex items-center gap-4 mt-6">
              ...
              <motion.div initial={{opacity: 0}} animate={{opacity: 1}} className='mt-6 bg-emerald-50 border border-emerald-200 p-5 rounded-2xl shadow-sm'>
                ...
                <button onClick={handleNext} className='w-full bg-gradient-to-r from-emerald-600 to-teal-500 text-white py-3 rounded-xl shadow-md hover:opacity-90 transition flex items-center justify-center gap-1'>Next Question <BsArrowRight size = {18}/> </button>
              </motion.div>
            )}
